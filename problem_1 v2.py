...
class DoublyLinkedList:
    ...
    def pop(self):
        ...
        return node.value
    ## assumes the node being passed in exists in the list so that operations are O(n)
    def move_to_tail(self, node):
        ##if already tail return (this covers case where single node and tail is also head)
        if self.tail == node:
            return self.tail
...
class LRU_Cache(object):
    ...
    def get(self, key):
        ...
        ##check if that key exists in hashmap.
